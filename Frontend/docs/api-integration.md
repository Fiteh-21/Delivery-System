...
// PATCH
await api.patch('/deliveries/1', { status: 'delivered' })

// DELETE
await api.delete('/deliveries/1')
```
...
```tsx
import api from '@/lib/api'
import type { Delivery, CreateDeliveryRequest } from '@/types'

export async function getDeliveries(): Promise<Delivery[]> {
  const response = await api.get<{ data: Delivery[] }>('/deliveries')
  return response.data.data
}

export async function createDelivery(data: CreateDeliveryRequest): Promise<Delivery> {
  const response = await api.post<{ data: Delivery }>('/deliveries', data)
  return response.data.data
}
```
